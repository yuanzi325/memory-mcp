...
async function readVaultRow() {
  ...
  if (error) throw toDbError("Supabase readVaultRow failed", error);
...
async function writeVaultState(nextState, { expectedUpdatedAt = "", rowExists = true } = {}) {
  ...
  if (rowExists) {
    ...
    if (updateError) throw toDbError("Supabase writeVaultState update failed", updateError);
  ...
  if (upsertError) throw toDbError("Supabase writeVaultState upsert failed", upsertError);
