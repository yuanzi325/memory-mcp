...
function buildVaultBriefing(stateJson, modules = [], limit = 3) {
  ...
  if (selected.includes("health")) {
    ...
    const items = [];
    if (health.cycleLength) items.push(`周期长度：${health.cycleLength} 天`);
    if (health.periodLength) items.push(`经期长度：${health.periodLength} 天`);
    const periods = ensureArray(health.periods || health.period);
    ...
    const recentLogs = sortByDateDesc(logs, (l) => l.date || "").slice(0, 2);
    for (const entry of recentLogs) {
      const note = compactText(entry.note || entry.content || "", 50);
